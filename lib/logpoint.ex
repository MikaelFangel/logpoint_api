defmodule Logpoint do
  ...
  def incident(pid, incident_obj_id, incident_id),
    do: Client.incident_info(pid, incident_obj_id, incident_id)

  def incidents(pid, start_time, end_time),
    do: Client.incident_info(pid, :incidents, start_time, end_time)

  def incident_states(pid, start_time, end_time),
    do: Client.incident_info(pid, :incident_states, start_time, end_time)
  ...
  def resolve_incidents(pid, indent_ids),
    do: Client.update_incidents(pid, :resolve, indent_ids)

  def close_incidents(pid, indent_ids),
    do: Client.update_incidents(pid, :close, indent_ids)

  def reopen_incidents(pid, indent_ids),
    do: Client.update_incidents(pid, :reopen, indent_ids)
